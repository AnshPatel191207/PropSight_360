...
class CommuteService {
  ...
  async generateAudit(origin, destination, listedTime) {
    try {
      const routeData = await this.getRouteAnalysis(origin, destination);
  ...
  async getRouteAnalysis(origin, destination) {
    ...
    const url = `https://api.tomtom.com/routing/1/calculateRoute/${startPos}:${endPos}/json?key=${this.tomtomApiKey}&traffic=true&report=effectiveSettings&instructionsType=text`;
  ...
  async getCoordinates(address) {
    try {
      const localizedAddress = address.toLowerCase().includes('ahmedabad') ? address : `${address}, Ahmedabad, Gujarat`;
      const url = `https://api.tomtom.com/search/2/geocode/${encodeURIComponent(localizedAddress)}.json?key=${this.tomtomApiKey}&limit=1&lat=23.0225&lon=72.5714&radius=50000`;
      
      const response = await axios.get(url);
      if (!response.data.results || response.data.results.length === 0) throw new Error(`Could not find: ${address}`);
